...
1. When configuring the Datadog Agent.
2. When sending custom metrics, logs, or traces to Datadog.

#### APP Key:

1. When writing scripts or using tools to interact with the Datadog API.
2. When automating the creation of dashboards, monitors, or other Datadog resources.
...
Logs are records of discrete events generated by systems.
...
Tracess represents the journey of a single request moves through the components of your system. ex, a user request travelling through a series of microservices.

#### Filter traces

For application traces, you can configure the Datadog APM agent to sample and filter traces. **DD_TRACE_SAMPLE_RATE=0.25**

## Metrics

Quantitative data points that provides insights of a system

#### Filter metrics

1. Exclude Specific Metrics


    ```
    dogstatsd_metrics:
      - exclude: "metric.name.to.exclude"
    ```

2. Exclude Metrics by Tag


    ```
    exclude_metrics_by_tags:
      - "env:dev"
      - "team:internal"
    ```

3. Use Metric Allowlist


    ```
    dogstatsd_metrics:
      - include: "my.metric.prefix.*"
    ```

## APM

what to do to enable is ?

- apm_config.enabled is set to true in the Agent configuration
- Download the latest dd-java-agent.jar file
- Add the Java agent as a JVM argument when starting your application.
